...
public:
    ...
    inline void next()
    { m_index = (++m_index >= m_systems.size() ? 0 : m_index); }

    inline void prev()
    { m_index = (--m_index < 0 ? m_systems.size() - 1 : m_index); }
    ...
    inline GameSystem *activeSystem() const
    { return m_active; }

    inline std::vector<GameSystem *> &systems()
    { return m_systems; }
};
